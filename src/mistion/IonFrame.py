...
class IonFrame:
    def __init__(
        ...
        self.nside = nside
        self.dlayer = DLayer(dt, position, dbot, dtop, ndlayers, nside, pbar, _autocalc)
        self.flayer = FLayer(dt, position, fbot, ftop, nflayers, nside, pbar, _autocalc)
    ...
    def plot_ded(self, gridsize=200, layer=None, title=None, cmap="viridis", **kwargs):
        barlabel = r"$m^{-3}$"
        if title is None:
            if layer is None:
    ...
    def plot_det(self, gridsize=200, layer=None, title=None, cmap="viridis", **kwargs):
        barlabel = r"$K^\circ$"
        if title is None:
            if layer is None:
    ...
    def plot_fed(self, gridsize=200, layer=None, title=None, cmap="viridis", **kwargs):
        barlabel = r"$m^{-3}$"
        if title is None:
            if layer is None:
    ...
    def plot_fet(self, gridsize=200, layer=None, title=None, cmap="viridis", **kwargs):
        barlabel = r"$K^\circ$"
        if title is None:
            if layer is None:
    ...
    def plot_datten(
        ...
        **kwargs,
        ...
        if title is None:
            title = (
                r"Average $(1 - f_{a})$ in the D layer at " + f"{freq / 1e6:.1f} MHz"
            )
        barlabel = None
        return polar_plot(
            (np.deg2rad(az), 90 - el, 1 - datten),
            dt=self.dt,
            pos=self.position,
            title=title,
            barlabel=barlabel,
    ...
    def plot_frefr(
        ...
        **kwargs
        ...
        if title is None:
            title = (
                r"Refraction $\delta \theta$ in the F layer at "
                + f"{freq / 1e6:.1f} MHz"
            )
        barlabel = r"$deg$"
        return polar_plot(
    ...
    def plot_troprefr(self, gridsize=200, title=None, cmap="viridis_r", **kwargs):
        el, az = elaz_mesh(gridsize)
